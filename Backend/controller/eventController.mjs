...
const handleUpdateImageFiles = (arrayField, files) => {
  if (files) {
    files.forEach((file, index) => {
      if (arrayField[index] && file) {
        const oldImagePath = path.resolve("uploads", arrayField[index].image);
        if (fs.existsSync(oldImagePath)) {
          fs.unlinkSync(oldImagePath);
        }
        arrayField[index].image = file.filename;
      }
    });
  }
};
export const updateEvent = async (req, res) => {
  try {
    ...
    // Example: parse and update fields
    const updatedData = {
    ...
    if (req.files) {
      ...
      if (mainImage) {
        updatedData.image = mainImage.filename;
      }

      // Handle subtitles, services, highlights here as needed
    }
